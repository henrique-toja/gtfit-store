// products.js (Updated with corrected accordion logic)
(function() {
    ...
    function generateAccordionItem(title, content, isOpen = false) {
        ...
        // A classe 'open' será usada pelo JS para controlar o estado
        return `
    ...
    // --- LÓGICA DO ACORDEÃO CORRIGIDA ---
    function addEventListenersForDetail() {
        // Itera sobre cada item do acordeão que foi renderizado
        document.querySelectorAll('.accordion-item').forEach(item => {
            const header = item.querySelector('.accordion-header');
            const content = item.querySelector('.accordion-content');
            const icon = header.querySelector('i');

            // Função para abrir ou fechar o item
            const toggleItem = () => {
                const isOpen = item.classList.contains('open');
                
                // Ação de abrir ou fechar baseada no estado atual
                if (isOpen) {
                    content.style.maxHeight = '0px'; // Fecha o item
                    icon.classList.remove('rotate-180');
                    item.classList.remove('open');
                } else {
                    // Abre o item definindo a altura máxima para a altura do seu conteúdo
            ...
            // Verifica se o item deve começar aberto por padrão
            if (item.classList.contains('open')) {
                // Atraso mínimo para garantir que o DOM foi renderizado e 'scrollHeight' é calculado corretamente
                setTimeout(() => {
                    content.style.maxHeight = content.scrollHeight + 'px';
                    icon.classList.add('rotate-180');
                }, 10);
